...
async function saveProduct(evt) {
    ...
    try {
        ...
        if (editingProduct && currentProductId) {
            ...
            url = `${API_BASE}/admin/products/${currentProductId}?clientID=${encodeURIComponent(clientID)}`;
            method = 'PUT';
        } else {
            // Create new product
            url = `${API_BASE}/admin/products?clientID=${encodeURIComponent(clientID)}`;
            method = 'POST';
        }
...
async function editProduct(productId) {
    ...
    try {
        ...
        // Updated to use RESTful endpoint
        const response = await fetch(`${API_BASE}/admin/products/${productId}?clientID=${encodeURIComponent(clientID)}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': token
...
async function archivePrice(priceId) {
    ...
    try {
        ...
        // Updated to use RESTful endpoint
        const response = await fetch(`${API_BASE}/admin/prices/${priceId}?clientID=${encodeURIComponent(clientID)}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': token
...
async function unsetDefault() {
    ...
    for (const p of prices) {
      // Updated to use RESTful endpoint
      await fetch(`${API_BASE}/admin/prices/${p.id}?clientID=${encodeURIComponent(clientID)}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': token
...
async function setDefault(priceId) {
    ...
    // Updated to use RESTful endpoint
    await fetch(`${API_BASE}/admin/prices/${priceId}?clientID=${encodeURIComponent(clientID)}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': token
