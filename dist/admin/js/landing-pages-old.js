...
async function loadAvailableOffers() {
    try {
        ...
        // Use the same API pattern as offers.js - POST with action
        const response = await fetch(window.CONFIG.apiBase, {
            method: 'POST',
            headers: {
...
async function publishLandingPage(evt) {
    ...
    try {
        ...
        const payload = {
            ...
            landingPageId: currentLandingPageId  // Backend expects camelCase
        };
        
        console.log('Publishing landing page with payload:', JSON.stringify(payload, null, 2));
