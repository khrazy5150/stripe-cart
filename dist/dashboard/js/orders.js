...
async function loadOrdersForClient() {
    ...
    try {
        ...
        // Updated to use authedFetch with /admin prefix
        const result = await authedFetch(`/admin/orders?${params.toString()}`, { 
            method: 'GET'
        });
        
        if (!result.ok) throw new Error(`HTTP ${result.status || 'error'}`);
...
async function fulfillOrder(orderId) {
    ...
    try {
        ...
        // Mark fulfilled - Updated to use authedFetch with /admin prefix
        const result = await authedFetch(`/admin/orders/${encodeURIComponent(orderId)}`, {
            method: 'PUT',
            body: { fulfilled: true }
        });
