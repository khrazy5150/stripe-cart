 ...
 * Sets display name, avatar, and stores user role
 */
async function loadUserProfile() {
  const cu = getCurrentUser();
  if (!cu) {
    document.getElementById('who').textContent = 'Unknown';
    return;
  }

  return new Promise((resolve, reject) => {
    // CRITICAL: Must get session first to ensure tokens are fresh
    cu.getSession((sessionErr, session) => {
      if (sessionErr || !session || !session.isValid()) {
        console.error('Failed to get valid session for user attributes:', sessionErr);
        document.getElementById('who').textContent = cu.getUsername() || 'Unknown';
        resolve();
        return;
      }

      // Now get user attributes with valid session
      cu.getUserAttributes((err, attributes) => {
        if (err || !attributes) {
        ...
        const userRole = attributes.find(attr => attr.Name === 'custom:role')?.Value || 'user'; // or 'custom:user_role'

        // Store role globally for access by other modules
        window.userRole = userRole;

        // Set display name
        const displayName = [firstName, lastName].filter(Boolean).join(' ') || cu.getUsername() || 'Unknown';
        ...
        if (avatarEl) {
          if (picture) {
            ...
            avatarEl.innerHTML = `<img src="${escapeHtml(picture)}" alt="${escapeHtml(displayName)}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 50%;">`;
          } else {
            // Use first letter of first name as fallback
            const initial = firstName ? firstName.charAt(0).toUpperCase() : (cu.getUsername() ? cu.getUsername().charAt(0).toUpperCase() : '?');
            avatarEl.textContent = initial;
            // Optional: clear any previous image if there was one
            avatarEl.innerHTML = initial;
          }
        }

        console.log('User profile loaded:', { displayName, userRole, hasPicture: !!picture });
        resolve();
      });
    });
  });
...
window.getUserRole = () => window.userRole || 'user';
...
async function authedFetch(pathOrUrl, options = {}) {
  ...
  const base = baseOverride || cfg.apiBase || '';
...
//   // 1. Check if the library is loaded. If not, retry after a slight delay.
//   if (!window.intlTelInput) {
//     console.warn('intlTelInput not yet defined. Retrying in 50ms...');
//     // Use setTimeout to yield execution and retry once the script has defined the global.
//     setTimeout(loadPhoneElement, 50); 
//     return;
//   }

//   // 2. If the library is defined, proceed with initialization (original code)
//   console.log('Loading phone element...');
  ...
  user.authenticateUser(auth, {
    onSuccess: async () => {
      ...
      setTimeout(async () => {
        ...
        await loadUserClientData(); // This still uses currentClientID variable
...
function handleSignOut() {
  ...
  window.userRole = null; // Clear user role
  
  if (window.resetOrdersData) {
    window.resetOrdersData();
  }

  if (window.resetDashboardStats) {
    window.resetDashboardStats('Sign in to view stats');
  }
