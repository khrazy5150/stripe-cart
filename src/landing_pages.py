...
# Single DynamoDB resource
_dynamodb = boto3.resource("dynamodb")

def get_stripe_keys_table():
    """Return the DynamoDB table for Stripe keys (strict; no fallback)."""
    return _dynamodb.Table(STRIPE_KEYS_TABLE)

def get_app_config_table():
    """Return the DynamoDB app-config table (strict; no fallback)."""
...
def get_landing_pages(event: Dict[str, Any], context) -> Dict[str, Any]:
    """GET /admin/landing-pages — Get all landing pages for a client"""
    try:
        ...
        if not item:
            return _ok({'landing_pages': [], 'count': 0})

        landing_pages = decimal_to_float(item.get('landing_pages', []))

        # Sort by created_at descending
        landing_pages.sort(key=lambda x: x.get('created_at', ''), reverse=True)

        return _ok({'landing_pages': landing_pages, 'count': len(landing_pages)})
...
def create_landing_page(event: Dict[str, Any], context) -> Dict[str, Any]:
    """POST /admin/landing-pages — Create a new landing page"""
    try:
        data = _parse_body(event)
        client_id = _extract_client_id(event)
        ...
        return _ok({'success': True, 'landing_page_id': landing_page_id, 'landing_page': landing_page})
...
def get_landing_page_details(event: Dict[str, Any], context, landing_page_id: str) -> Dict[str, Any]:
    """GET /admin/landing-pages/{landing_page_id} — Get details for a specific landing page"""
    try:
        client_id = _extract_client_id(event)
        if not client_id:
            return _err("clientID required", 400)
...
def update_landing_page(event: Dict[str, Any], context, landing_page_id: str) -> Dict[str, Any]:
    """PUT /admin/landing-pages/{landing_page_id} — Update an existing landing page"""
    try:
        data = _parse_body(event)
        client_id = _extract_client_id(event)
        ...
        return _ok({'success': True, 'landing_page_id': landing_page_id})
...
def archive_landing_page(event: Dict[str, Any], context, landing_page_id: str) -> Dict[str, Any]:
    """DELETE /admin/landing-pages/{landing_page_id} — Archive (soft delete) a landing page"""
    try:
        client_id = _extract_client_id(event)
        if not client_id:
            return _err("clientID required", 400)
...
def check_plan_limits(client_id: str, resource_type: str) -> Dict[str, Any]:
    """Check if client can create more of a resource based on their plan (strict env)."""
    try:
        # Get app-config for plan limits (strict, no fallback)
        config_table = get_app_config_table()

        config_response = config_table.get_item(
            Key={'config_key': 'landing_page_config', 'environment': ENVIRONMENT}
        )
        ...
        # Default to allowing if the plan check fails
        return {'allowed': True}
