...
from config_loader import load_config, get_config_value, get_offers_base_url
...
def _ok(body: Dict[str, Any], code: int = 200) -> Dict[str, Any]:
    return {"statusCode": code, "headers": _json_headers(), "body": json.dumps(body, cls=DecimalEncoder)}
...
def load_stripe_tenant_with_offer(event: Dict[str, Any]) -> Tuple[str, str, Dict, str, str, str, str]:
    ...
    Uses config_loader for base URL.
    """
    ...
    # Use config_loader for base URL (falls back to tenant-specific if needed)
    base_frontend_url = get_offers_base_url(client_id, mode)
    if not base_frontend_url:
        base_frontend_url = get_config_value('frontend_base_url', 'https://juniorbay.com')
        test_or_prod = get_config_value('frontend_test_dir' if mode == 'test' else 'frontend_prod_dir', 'dist')
        base_frontend_url = f"{base_frontend_url}/{test_or_prod}/{client_id}/"
...
def create_checkout_session(event: Dict[str, Any], context) -> Dict[str, Any]:
    ...
    try:
        ...
        # Get product info and determine if shipping is needed
        if product_id:
            product = stripe.Product.retrieve(product_id)
            product_type = product.metadata.get("product_type", "physical")
            product_category = product.metadata.get("product_category", "standard")
        else:
        ...
        # Determine success URL based on category
        success_url = data.get("successUrl")
        cancel_url = data.get("cancelUrl")
        
        if not success_url:
            if product_category == "basic":
        ...
        if product_type == "physical":
            session_data["shipping_address_collection"] = {"allowed_countries": ["US", "CA"]}
...
def process_one_click_upsell(event: Dict[str, Any], context) -> Dict[str, Any]:
    ...
    try:
        ...
        if not original_pi_id or not product_id or not customer_id:
            return _err("Missing required parameters")
        ...
        if not pm_to_use:
            return _err("No saved payment method available", 409)
        ...
        return _err(f"Failed to process upsell: {str(e)}")
...
def _process_payment_intent(payment_intent, client_id: str, mode: str):
    """Process payment intent and save order with comprehensive phone extraction."""
    try:
        ...
        if not customer_phone:
            ...
            if cust_id:
                try:
                    ...
                    logger.warning(f"Customer retrieve failed: {str(e)}")
        ...
        logger.info(f"Phone resolved from {source}, value={customer_phone}")
...
def get_client_offers(event: Dict[str, Any], context) -> Dict[str, Any]:
    ...
    try:
        ...
        if not sk:
            return _err(f"Failed to decrypt Stripe secret key")
...
def update_offer(event: Dict[str, Any], context) -> Dict[str, Any]:
    ...
    try:
        ...
        return _ok({"success": True, "offer": offer_config, "client_id": client_id})
    ...
    try:
        ...
        for product in products.data:
            ...
            product_list.append({
                ...
                "product_type": meta.get("product_type", "physical"),
                "product_category": meta.get("product_category", "standard"),
    ...
    try:
        ...
        if shipping_config and shipping_config.get("api_key"):
            ...
            if not api_key.startswith("ENCRYPTED(") and api_key != "****":
                try:
                    encrypted = _kms_encrypt_wrapped(api_key.encode('utf-8'))
...
# --------- Product Management ---------
def create_stripe_product(event: Dict[str, Any], context) -> Dict[str, Any]:
    """Create a new Stripe product with metadata and pricing."""
    try:
        client_id, mode, offer_config, pk, sk, wh, full_offer_url = load_stripe_tenant_with_offer(event)
        stripe.api_key = sk
        ...
        metadata = {
            ...
            "package_length": str(data.get("package_length", 10)),
            "package_width": str(data.get("package_width", 8)),
            "package_height": str(data.get("package_height", 4)),
            "package_weight": str(data.get("package_weight", 1)),
        }
        
        # Add upsell configuration if provided
        if data.get("upsell_product_id"):
            metadata["upsell_product_id"] = data["upsell_product_id"]
        if data.get("upsell_price_id"):
            metadata["upsell_price_id"] = data["upsell_price_id"]
        if data.get("upsell_offer_text"):
            metadata["upsell_offer_text"] = data["upsell_offer_text"]
        if data.get("upsell_savings_text"):
            metadata["upsell_savings_text"] = data["upsell_savings_text"]
        
        # Add page configuration metadata
        if data.get("hero_title"):
            metadata["hero_title"] = data["hero_title"]
        if data.get("hero_subtitle"):
            metadata["hero_subtitle"] = data["hero_subtitle"]
        if data.get("benefits"):
            metadata["benefits"] = data["benefits"]  # pipe-separated string
        if data.get("guarantee"):
            metadata["guarantee"] = data["guarantee"]
        if data.get("shipping"):
            metadata["shipping"] = data["shipping"]
        if data.get("color_scheme"):
            metadata["color_scheme"] = data["color_scheme"]
        if data.get("countdown_minutes"):
            metadata["countdown_minutes"] = str(data["countdown_minutes"])
        if data.get("urgency_message"):
            metadata["urgency_message"] = data["urgency_message"]
        ...
        # Create price(s) if provided
        prices = []
        for price_data in data.get("prices", []):
            if not price_data.get("unit_amount"):
                continue
            ...
            if price_data.get("recurring"):
                price_params["recurring"] = {
                    ...
                    "interval_count": price_data["recurring"].get("interval_count", 1)
                }
        ...
        logger.exception("Stripe error creating product")
        return _err(f"Stripe error: {str(e)}")
    except Exception as e:
        logger.exception("Error creating product")
        return _err(f"Failed to create product: {str(e)}")


def update_stripe_product(event: Dict[str, Any], context) -> Dict[str, Any]:
    """Update an existing Stripe product and its metadata."""
    try:
        client_id, mode, offer_config, pk, sk, wh, full_offer_url = load_stripe_tenant_with_offer(event)
        stripe.api_key = sk
        ...
        # Handle images so they can be deleted from Stripe
        # Sending an empty array of images [] will be ignored by Stripe
        if "images" in data:
            imgs = data["images"]
            if len(imgs) == 0:
                logger.info("Images are empty")
                update_data["images"] = []
            elif isinstance(imgs, list):
                update_data["images"] = imgs                 
            elif imgs in (None, "", False):
                update_data["images"] = []                  
            else:
                logger.warning(f"Ignoring non-list 'images' payload: {type(imgs)}")

        # logger.info("Handling images")
        # if "images" in data:
        #     imgs = data["images"]
        #     logger.info(f"Images: {imgs}")
        #     # Normalize: only lists are valid; empty list clears images in Stripe
        #     if isinstance(imgs, list):
        #         update_data["images"] = imgs
        #         logger.info("Setting images to 'imgs' value")
        #     elif imgs in (None, "", False):
        #         update_data["images"] = []  # treat empty/falsey as "clear"
        #         logger.info("Setting images to empty list")
        #     else:
        #         # Defensive: ignore invalid shapes rather than sending junk to Stripe
        #         logger.warning(f"Ignoring non-list 'images' payload: {type(imgs)}")
        
        # Build metadata updates
        metadata_updates = {}
        metadata_fields = [
            "product_type", "product_category", "package_length", "package_width", "package_height",
            "package_weight", "upsell_product_id", "upsell_price_id",
            "upsell_offer_text", "upsell_savings_text", "hero_title",
            "hero_subtitle", "benefits", "guarantee", "shipping",
            "color_scheme", "countdown_minutes", "urgency_message"
        ]
        
        for field in metadata_fields:
            if data.get(field) is not None:
                metadata_updates[field] = str(data[field])
        ...
        logger.info(f"Updated product {product.id} images -> {product.images}")
        ...
        logger.exception("Stripe error updating product")
        return _err(f"Stripe error: {str(e)}")
    except Exception as e:
        logger.exception("Error updating product")
        return _err(f"Failed to update product: {str(e)}")


def create_stripe_price(event: Dict[str, Any], context) -> Dict[str, Any]:
    ...
    try:
        ...
        if not product_id:
            return _err("product_id is required")
        if not data.get("unit_amount"):
            return _err("unit_amount is required")
        ...
        if data.get("nickname"):
            price_params["nickname"] = data["nickname"]
        if data.get("metadata"):
            price_params["metadata"] = data["metadata"]
        if data.get("active") is not None:
            price_params["active"] = data["active"]
        
        # Handle recurring prices
        if data.get("recurring"):
            price_params["recurring"] = {
                "interval": data["recurring"].get("interval", "month"),
                "interval_count": data["recurring"].get("interval_count", 1)
            }
        ...
        logger.exception("Stripe error creating price")
        return _err(f"Stripe error: {str(e)}")
    except Exception as e:
        logger.exception("Error creating price")
        return _err(f"Failed to create price: {str(e)}")


def update_stripe_price(event: Dict[str, Any], context) -> Dict[str, Any]:
    """Update a Stripe price (limited fields - most are immutable)."""
    try:
        client_id, mode, offer_config, pk, sk, wh, full_offer_url = load_stripe_tenant_with_offer(event)
        stripe.api_key = sk
        ...
        if not price_id:
            return _err("price_id is required")
        ...
        return _err(f"Price {price_id} not found", 404)
    except stripe.error.StripeError as e:
        logger.exception("Stripe error updating price")
        return _err(f"Stripe error: {str(e)}")
    except Exception as e:
...
def delete_stripe_product(event: Dict[str, Any], context) -> Dict[str, Any]:
    ...
    try:
        ...
        # Stripe doesn't allow true deletion, so we archive it
        ...
        logger.exception("Stripe error archiving product")
        return _err(f"Stripe error: {str(e)}")
    except Exception as e:
        logger.exception("Error archiving product")
        return _err(f"Failed to archive product: {str(e)}")


def get_product_details(event: Dict[str, Any], context) -> Dict[str, Any]:
    """Get detailed product information including all prices."""
    try:
        client_id, mode, offer_config, pk, sk, wh, full_offer_url = load_stripe_tenant_with_offer(event)
        stripe.api_key = sk
        ...
        logger.exception("Stripe error fetching product")
        return _err(f"Stripe error: {str(e)}")
    except Exception as e:
        logger.exception("Error fetching product details")
        return _err(f"Failed to fetch product: {str(e)}")

# ---------- router ----------

def lambda_handler(event, context):
    """Main router for stripe cart requests."""
    try:
        ...
        if method == "GET":
            if '/admin/shipping-config' in path:
        ...
        if method == "POST":
            ...
            # NEW: Product management actions
            elif action == "create_product":
                return create_stripe_product(event, context)
            elif action == "update_product":
                return update_stripe_product(event, context)
            elif action == "get_product_details":
                return get_product_details(event, context)
            elif action == "delete_product":
                return delete_stripe_product(event, context)
            elif action == "create_price":
                return create_stripe_price(event, context)
            elif action == "update_price":
                return update_stripe_price(event, context)
