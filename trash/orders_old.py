...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """Main handler for order management endpoints."""
    headers = {
...
def format_address(address: Dict[str, Any]) -> str:
    """Format address dict as readable string."""
...
def get_client_id_from_event(event):
    """
    Extract clientID from multiple sources:
...
def handle_get_orders(event, headers):
    ...
    try:
        ...
        # Get clientID from multiple sources
        ...
        if client_id:
            ...
            # Special case for legacy client
            if client_id == 'f8810370-7021-7011-c0cc-6f22f52954d3':
                filter_expr = (
                    Attr('fulfilled').eq('false') & 
                    (Attr('client_id').eq(client_id) | Attr('client_id').not_exists())
                )
            else:
                filter_expr = (
                    Attr('fulfilled').eq('false') & 
