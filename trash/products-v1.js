...
function showProductForm(productData = null) {
    ...
    if (productData) {
        ...
        // Upsell config
        document.getElementById('upsellProductId').value = meta.upsell_product_id || '';
        document.getElementById('upsellPriceId').value = meta.upsell_price_id || '';
        document.getElementById('upsellOfferText').value = meta.upsell_offer_text || '';
        ...
        setTimeout(() => {
            ...
            // THEN show existing images as previews
...
async function saveProduct(evt) {
    ...
    try {
        ...
        // if (!editingProduct && pricesToAdd.some(p => p.metadata?.is_default === "true")) {
        //     await unsetDefault(); // make sure only one default exists
        // }
        ...
        <div class="product-details-view">
            ...
            ${meta.upsell_product_id ? `
                <div class="mt16">
                    ...
                    <div class="muted mt8">
                        <div>Product ID: ${meta.upsell_product_id}</div>
                        <div>Price ID: ${meta.upsell_price_id || 'Not set'}</div>
