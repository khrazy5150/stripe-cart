...
def _require_client_id(event):
    ...
    2. Cognito claims (sub, cognito:username, email)
    """
    try:
...
def _kms_decrypt_wrapped(blob: str) -> str:
    ...
    Supports ENCRYPTED() wrapper format.
    """
...
def lambda_handler(event, context):
    """
    POST /admin/verify
    Verify Stripe credentials for a client.
    
    Body: {
        "clientID": "xxx",  // optional, defaults to authenticated user
        "mode": "test" | "live"  // optional, defaults to tenant's current mode
    }
    
    Returns: {
        "clientID": "xxx",
        "mode": "test" | "live",
        "publishable_key_ok": true/false,
        "secret_key_ok": true/false,
        "webhook_secret_ok": true/false,
        "stripe_account": "acct_xxx",  // if secret key works
        "notes": ["array of status messages"]
    }
    """
    try:
        ...
        # Get client ID (validates auth too)
        try:
